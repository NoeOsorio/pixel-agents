  ...
  try {
    ...
    if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
      ...
      if (hasToolUse) {
        ...
        target?.postMessage({ type: 'agentStatus', id: agentId, status: 'active' });
        ...
        if (hasNonExemptTool) {
          startPermissionTimer(agentId, agents, permissionTimers, PERMISSION_EXEMPT_TOOLS, target);
        }
      } else if (blocks.some((b) => b.type === 'text') && !agent.hadToolsInTurn) {
        startWaitingTimer(agentId, TEXT_IDLE_DELAY_MS, agents, waitingTimers, target);
      }
    } else if (record.type === 'progress') {
      processProgressRecord(agentId, record, agents, permissionTimers, target);
      ...
      if (agent.activeToolIds.size > 0) {
        ...
        target?.postMessage({ type: 'agentToolsClear', id: agentId });
      }
